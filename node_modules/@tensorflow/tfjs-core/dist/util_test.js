...
describe('util.fetch', function () {
    it('should allow overriding global fetch', function () {
        var savedFetch = environment_1.ENV.global.fetch;
        environment_1.ENV.global.fetch = function () { };
        spyOn(environment_1.ENV.global, 'fetch').and.callThrough();
        util.fetch('');
        expect(environment_1.ENV.global.fetch).toHaveBeenCalled();
        environment_1.ENV.global.fetch = savedFetch;
    });
});
jasmine_util_1.describeWithFlags('util.fetch node', jasmine_util_1.NODE_ENVS, function () {
    it('should use node-fetch', function () {
        var savedFetch = util.systemFetch;
        var savedGlobalFetch = environment_1.ENV.global.fetch;
        // @ts-ignore
        environment_1.ENV.global.fetch = null;
        // @ts-ignore
        util.systemFetch = null;
        spyOn(util.getNodeFetch, 'fetchImport').and.callFake(function () { return function () { }; });
        util.fetch('');
        // tslint:disable-next-line:no-any
        expect(util.getNodeFetch.fetchImport).toHaveBeenCalled();
        // @ts-ignore
        environment_1.ENV.global.fetch = savedGlobalFetch;
        // @ts-ignore
